...
const CACHE_FILE = '/tmp/luongson-m3u-cache.txt';
...
async function resolveStreamUrl(slug) {
  try {
    const data = await callAPI(`${LS_API}/match-detail-slug?slug=${encodeURIComponent(slug)}`);
    if (!data?.value?.success || !data?.value?.datas) return null;
    const d = data.value.datas;
    return { hls: d.linkLive, flv: d.linkLiveFlv, name: `${d.homeName} vs ${d.awayName}`, logo: d.homeLogo };
  } catch(e) { return null; }
}

async function generateFastPlaylist(baseUrl = 'https://xoilac-iptv.vercel.app') {
  const matches = await getAllMatches();
  let m3u = '#EXTM3U\n';
  m3u += '#PLAYLIST:LuongSonTV - Live Sports\n';
  m3u += `# Generated: ${new Date().toISOString()}\n`;
  m3u += '# Fast redirect playlist — click match to load stream\n';
  m3u += '# Streams served via ' + baseUrl + '/api/ls-stream?slug=...\n\n';

  let count = 0;
  for (const m of matches) {
    if (!m.slugUrl) continue;
    const group = m.liveGame ? '\uD83D\uDD34 Live Now' : (m.leagueName || 'Matches');
    const time = m.matchTime ? new Date(m.matchTime * 1000).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) : '';
    const blvInfo = m.commentator ? ` [${m.commentator}]` : '';
    const score = m.liveGame && m.homeScore != null ? ` (${m.homeScore}:${m.awayScore})` : '';
    const title = `${m.homeName} vs ${m.awayName}`;
    const logo = m.homeLogo || '';
    m3u += `#EXTINF:0 tvg-id="ls-${m.matchId}" tvg-name="${title}" tvg-logo="${logo}" group-title="${group}",${title}${score} ${time}${blvInfo}\n`;
    m3u += `${baseUrl}/api/ls-stream?slug=${encodeURIComponent(m.slugUrl)}\n`;
    count++;
  }
...
async function generatePlaylistCached(ttlMs = CACHE_TTL) {
  try {
  ...
  const fresh = await generatePlaylist();
  fs.writeFileSync(CACHE_FILE, fresh, 'utf8');
  return fresh;
}

module.exports = { generatePlaylist, generatePlaylistCached, generateFastPlaylist, resolveStreamUrl, getAllMatches, getMatchDetail };
